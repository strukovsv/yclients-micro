...
with active_cards as (
  select
    ...
    (st1.js->>'cost')::float as costs,
     c1.js js
  from cards c1
  join storage_transactions st1
    on st1.id = (c1.js->>'goods_transaction_id')::int
  where (
         (
          c1.js->'status'->>'title' = 'Активирован'
          and
          (c1.js->>'expiration_date')::date >= current_date
         ) or (
           c1.js->'status'->>'title' = 'Выпущен'
           and
           (c1.js->>'created_date')::date > (current_date - 30)
         )
        )
)
select
  ...
  sum((ac.js->>'united_balance_services_count')::int)
    as "[card] Занятий по абонементам, клв",
  min((ac.js->>'created_date')::date)
    as "[card] Покупка абонементов, дата",
  to_char(min((ac.js->>'created_date')::date), 'YYYY-MM')
    as "[card] Покупка абонементов, месяц",
  min((ac.js->>'activated_date')::date)
    as "[card] Активация абонементов, дата",
  to_char(min((ac.js->>'activated_date')::date), 'YYYY-MM')
    as "[card] Активация абонементов, месяц",
  max((ac.js->>'expiration_date')::date)
    as "[card] Окончание абонементов, дата",
  to_char(max((ac.js->>'expiration_date')::date), 'YYYY-MM')
    as "[card] Окончание абонементов, месяц",
  sum(ac.costs)
    as "[card] Сумма продажи, руб",
  count(*)
    as "[card] Активных абонементов, клв"
...
select
  ...
  concat(
    case
      when length(cl.js->>'phone') = 12 then left(cl.js->>'phone', 5)
        || '•••' || right(cl.js->>'phone', 4)
      else cl.js->>'phone'
    end,
    ' ',
...
select
  ...
  to_char(w.closed_at::date, 'YYYY-MM')
    as "[workflow] Воронка завершена, месяц",
  ...
  to_char(ws.created_at::date, 'YYYY-MM')
    as "[workflow] Открыта задача, месяц",
  to_char(ws.created_at::date, 'YYYY') as "[workflow] Открыта задача, год",
  to_char(ws.started_at::timestamp, 'DD.MM.YYYY HH24:MI:SS')
    as "[workflow] Запустить задачу в",
  EXTRACT(DAY FROM (coalesce(w.closed_at::timestamp, current_timestamp)
                    - (w.opened_at::timestamp)))
    as "[workflow] Длительность, дней",
  to_char((w.closed_at::timestamp) - (w.opened_at::timestamp),
          'FMDD "дней" HH24:MI:SS')
    as "[workflow] Длительность"
from workflow2 w
, workflow_stages2 ws
, detail_clients2 dc
where ws.id = (
  select max(id)
  from workflow_stages2 ws
